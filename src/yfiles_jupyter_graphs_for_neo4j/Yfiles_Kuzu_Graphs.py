...
class KuzuGraphWidget:
    ...
    def __init__(
        ...
        widget_layout: Optional[Any] = None,
        overview_enabled: Optional[bool] = False,
        context_start_with: Optional[str] = None,
        license: Optional[Dict] = None,
        autocomplete_relationships: Optional[bool] = False,
        layout: Optional[str] = "organic",
    ):
        """
    ...
    def set_autocomplete_relationships(
        self, autocomplete_relationships: Union[bool, str, list[str]]
    ) -> None:
        """
        ...
        if not isinstance(autocomplete_relationships, (bool, str, list)):
            raise ValueError(
                "autocomplete_relationships must be a bool, a string, or a list of strings"
            )
    ...
    def _get_relationship_types_expression(self) -> str:
        if (
            isinstance(self._autocomplete_relationships, list)
            and len(self._autocomplete_relationships) > 0
        ):
            return "AND type(rel) IN $relationship_types"
        return ""

    def show_cypher(
        self, cypher: str, layout: Optional[str] = None, **kwargs: Dict[str, Any]
    ) -> None:
        """
        ...
        widget = GraphWidget(
            overview_enabled=self._overview,
            context_start_with=self._context_start_with,
            widget_layout=self._layout,
            license=self._license,
            graph=G
            # self._session.run(cypher, **kwargs).graph(),
        )
        ...
        self.__apply_heat_mapping(
            {**self._node_configurations, **self._edge_configurations}, widget
        )
    ...
    def __get_neo4j_item_text(element: Dict) -> Union[str, None]:
        lowercase_element_props = {
            key.lower(): value for key, value in element.get("properties", {}).items()
        }
    ...
    @staticmethod
    def __configuration_mapper_factory(
        binding_key: str,
        configurations: Dict[str, Dict[str, str]],
        default_mapping: Callable,
    ) -> Callable[[int, Dict], Union[Dict, str]]:
        """
        ...
        def mapping(index: int, item: Dict) -> Union[Dict, str]:
            label = item["properties"][
                "_label"
            ]  # yjg stores the neo4j node/relationship type in properties["label"]
            if (
                label in configurations or "*" in configurations
                ...
                # property name
                    ...
                    not isinstance(type_configuration[binding_key], dict)
                    and type_configuration[binding_key] in item["properties"]
                ):
            ...
            if binding_key == "label":
                return KuzuGraphWidget.__get_neo4j_item_text(item)
            else:
                ...
                if (
                    len(parameters) > 1
                    and parameters[list(parameters)[0]].annotation == int
                ):
    ...
    def __apply_heat_mapping(self, configuration, widget: GraphWidget) -> None:
        setattr(
            widget,
            "_heat_mapping",
            KuzuGraphWidget.__configuration_mapper_factory(
                "heat", configuration, getattr(widget, "default_heat_mapping")
            ),
        )
    ...
    def __create_group_nodes(self, configurations, widget: GraphWidget) -> None:
        ...
        for group_label in group_node_properties.union(group_node_values):
            node = {
                "id": "GroupNode" + group_label,
                "properties": {"_label": group_label},
            }
    ...
    def __apply_parent_mapping(self, widget: GraphWidget) -> None:
        ...
        for edge in widget.edges[:]:
            rel_type = edge["properties"]["_label"]
        ...
        widget.edges = [
            edge for edge in widget.edges if edge["id"] not in edge_ids_to_remove
        ]
        current_parent_mapping = getattr(widget, "_node_parent_mapping")
        setattr(
            widget,
            "_node_parent_mapping",
            lambda index, node: node_to_parent.get(
                node["id"], current_parent_mapping(index, node)
            ),
        )
    ...
    def __apply_node_mappings(self, widget: GraphWidget) -> None:
        for key in POSSIBLE_NODE_BINDINGS:
            ...
            setattr(
                widget,
                f"_node_{key}_mapping",
                KuzuGraphWidget.__configuration_mapper_factory(
                    key, self._node_configurations, default_mapping
                ),
            )
        # manually set parent configuration
        setattr(
            widget,
            f"_node_parent_mapping",
            KuzuGraphWidget.__configuration_mapper_factory(
                "parent_configuration", self._node_configurations, lambda node: None
            ),
        )
    ...
    def __apply_edge_mappings(self, widget: GraphWidget) -> None:
        for key in POSSIBLE_EDGE_BINDINGS:
            ...
            setattr(
                widget,
                f"_edge_{key}_mapping",
                KuzuGraphWidget.__configuration_mapper_factory(
                    key, self._edge_configurations, default_mapping
                ),
            )

    def add_node_configuration(
        self, label: Union[str, list[str]], **kwargs: Dict[str, Any]
    ) -> None:
        """
    ...
    # noinspection PyShadowingBuiltins
    def add_relationship_configuration(
        self, type: Union[str, list[str]], **kwargs: Dict[str, Any]
    ) -> None:
        """
    ...
    # noinspection PyShadowingBuiltins
    def add_parent_relationship_configuration(
        self, type: Union[str, list[str]], reverse: Optional[bool] = False
    ) -> None:
        """
    ...
    def del_node_configuration(self, label: Union[str, list[str]]) -> None:
        ...
        if isinstance(label, list):
            for l in label:
                KuzuGraphWidget.__safe_delete_configuration(
                    l, self._node_configurations
                )
        else:
            KuzuGraphWidget.__safe_delete_configuration(
                label, self._node_configurations
            )
    ...
    def del_relationship_configuration(self, type: Union[str, list[str]]) -> None:
        ...
        if isinstance(type, list):
            for t in type:
                KuzuGraphWidget.__safe_delete_configuration(
                    t, self._edge_configurations
                )
        else:
            KuzuGraphWidget.__safe_delete_configuration(
                type, self._edge_configurations
            )
    ...
    # noinspection PyShadowingBuiltins
    def del_parent_relationship_configuration(
        ...
        if isinstance(type, list):
            self._parent_configurations = {
                rel_type
                for rel_type in self._parent_configurations
                if rel_type[0] not in type
            }
        else:
            self._parent_configurations = {
                rel_type
    ...
    def get_selected_node_ids(
        self, widget: Optional[Type["KuzuGraphWidget"]] = None
    ) -> List[str]:
        """
    ...
    def get_selected_relationship_ids(
        self, widget: Optional[Type["KuzuGraphWidget"]] = None
    ) -> List[str]:
        """
    ...
    node_cell_mapping = property(
        get_node_cell_mapping, set_node_cell_mapping, del_node_cell_mapping
    )
